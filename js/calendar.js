...
//   - 课程库:Supabase id='courses',结构 { list: [{id, name, teacher}] }
//
...
const CalendarApp = {
  ...
  render() {
    ...
    const offset = firstDay === 0 ? 6 : firstDay - 1; // 周一作为一周开始
  ...
  _renderDetail(dStr) {
    ...
    if (list.length === 0) {
      ...
      list.forEach((c) => {
        const course = this.getCourse(c.courseId);
        if (!course) {
          html += `<div class="cal-class-item" style="background:#eee;color:#888;">(课程已删除)</div>`;
          return;
        }
        const teach = TEACHERS[course.teacher];
        const end = c.start ? addHours(c.start, 2) : '';
        const realIdx = (s.classes[dStr] || []).indexOf(c);
        html += `<div class="cal-class-item" style="background:${teach.color};color:${teach.textColor}">
  ...
  async addCourse() {
    ...
    this.closeModal();
    this.openManage();  // 重新打开管理弹窗,显示更新后的列表
  },
